...
function Event() {
  ...
  useEffect(() => {
    const updateEvent = events.filter(
      (item) => item.eventname === eventname
    )[0];
    if (updateEvent) {
      setEventdata({
        id: updateEvent._id,
        club: updateEvent.clubname,
        image: updateEvent.eventimage,
        date: updateEvent.eventdate,
        name: updateEvent.eventname,
        desc: updateEvent.description,
      });
    }
  }, [events]);
        ...
        <Card shadow={true} className="mx-auto lg:px-16 pb-10">
          ...
          >
            <div>
              ...
              >
                <img
                  src={eventdata.image ? thumps : upload}
                  alt="update image"
